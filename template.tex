...
Acoustic training contains two stages: GMM-HMM framework based on maximum likelihood and hybrid DNN-HMM
...
standard setup in Kaldi recipes. The resulting number of physical GMM states from the
...
information. From architecture point of view, deploying a large scale
...
Based on the alignments provided by GMM stages above, a time-delayed neural network (TDNN, \cite{tdnn}) with multi-splicing is configured. It has 8 layers in total with 1280 hidden units in each layer. It is, as described in \cite{tdnn}, not fully connected as multi-layer perceptron - the input of each hidden layer is a frame-wise spliced output of its preceding layer. The input feature is high resolution MFCC with cepstral normalization plus pitches, which makes its dimension 43. Note that for each frame-wise input a 100-dimensional i-vector is also attached. This indicates that different from GMM-level training, we encode speaker information here to produce a stronger baseline for research community. The network is trained using lattice-free maximum mutual information (LFMMI, \cite{lfmmi}) as the objective function. More configurations about the DNN training itself can be found at \cite{lfmmi} and the AISHELL-2 example in Kaldi repository \footnote{https://github.com/kaldi-asr/kaldi/egs/aishell2}.
...
scripts. Out-of-vocabulary (OOV) words are mapped as $<$UNK$>$. The language
